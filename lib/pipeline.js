...
async function run(orgs, dateFrom, dateTo, clientName, env, cb, sectionCb) {
  ...
  sectionCb?.({ id: 'sov', title: 'AQ Press Analytics', summary: `${totalArts} total articles — ${orgs.slice(0,5).map(o=>`${o}: ${(arts[o]||[]).length}`).join(', ')}${orgs.length>5?' +more':''}` });
  ...
  sectionCb?.({ id: 'social', title: 'Social & YouTube', summary: orgs.slice(0,4).map(o=>{const s=socialResults[o]||{};return `${o}: X ${s.x?.aqPosts||0} · IG ${s.ig?.aqPosts||0} · LI ${s.li?.totalPosts||0} · YT ${s.yt?.videoCount||0}`;}).join(' | ') });
  ...
  sectionCb?.({ id: 'topics', title: 'Topic Ownership', summary: topTopics || 'Classification complete — no dominant topics found' });
  ...
  sectionCb?.({ id: 'momentum', title: 'Coverage Momentum', summary: `${spikeAnnotations.length} coverage spike${spikeAnnotations.length!==1?'s':''} detected across the tracked period` });
  ...
  sectionCb?.({ id: 'aeo', title: 'AEO / LLM Visibility', summary: `LLM mention counts — ${aeoLine||'No AEO data (optional keys not set)'}` });
  ...
  sectionCb?.({ id: 'gaps', title: 'White-Space Gaps', summary: emerging.length > 0 ? `${emerging.length} emerging gap${emerging.length!==1?'s':''}: ${emerging.map(e=>e.topic).join(', ')}` : 'No significant coverage gaps found' });
  ...
  sectionCb?.({ id: 'exec', title: 'Executive Summary', summary: execFindings[0]?.headline || 'Executive analysis complete' });
  const actions      = await genActionMatrix(orgs, data, claudeKey, tracker, cb);
  sectionCb?.({ id: 'actions', title: 'Action Matrix', summary: `${actions.length} strategic recommendation${actions.length!==1?'s':''} across ${orgs.length} organisation${orgs.length!==1?'s':''}` });
