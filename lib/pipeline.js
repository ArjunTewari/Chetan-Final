...
async function findGaps(orgs, dateFrom, dateTo, serperKey, claudeKey, arts, tracker, cb) {
  const anthropic = new Anthropic({ apiKey: claudeKey });
  // Gather all tracked article titles
  const trackedTitles = orgs.flatMap(o => (arts[o]||[]).map(a => a.title||'')).filter(Boolean);

  // Fetch general AQ India news via Serper (not org-specific)
  let generalArts = [];
  if (serperKey) {
    try {
      const res = await axios.post('https://google.serper.dev/news', {
        q:   'air quality pollution India AQI PM2.5 NCAP',
        gl:  'in',
        hl:  'en',
        num: 20,
      }, {
...
For EACH organisation, generate EXACTLY 4 action rows — one for each of the four priority types below. No org should have fewer or more than 4 rows.

Priority types (use each exactly once per org):
- "Fix Now" — urgent gap that must be addressed immediately
- "Leverage" — exploit an existing strength to compound impact
- "Optimise" — improve a current activity that is underperforming
- "Invest" — build a new capability not yet present

Area options: "Topics", "Media", "Social", "AEO", "Narrative", "Data", "Partnerships"

Return a JSON array of objects. For ${orgs.length} orgs × 4 rows = ${orgs.length * 4} total objects:
{"org": "Organisation name", "priority": "Fix Now|Leverage|Optimise|Invest", "area": "Topics|Media|Social|AEO|Narrative|Data|Partnerships", "action": "Specific 1-2 sentence action with concrete targets", "rationale": "Data-grounded reason citing specific numbers from the data above"}

Return ONLY a valid JSON array, no other text. Each org must appear exactly 4 times with all four priority types covered.`;
  try {
    const msg = await anthropic.messages.create({
      model: SONNET,
      max_tokens: 4000,
 ...
 */
async function run(orgs, dateFrom, dateTo, clientName, env, cb, sectionCb) {
  const tracker = createTracker();
  const claudeKey     = env.CLAUDE_KEY;
  const apidirKey     = env.APIDIRECT_KEY;
  const serperKey     = env.SERPER_KEY;
  ...
  cb?.(`  Keys: CLAUDE=${claudeKey?'✓':'✗'} SERPER=${serperKey?serperKey.slice(0,6)+'…':'✗ MISSING'} APIDIRECT=${apidirKey?'✓':'✗'} YT=${ytKey?'✓':'✗'}`);

  // 1. News collection
  cb?.('[1/8] Collecting news (Serper News Index)…');
  const arts = await newsCollector.run(orgs, dateFrom, dateTo, serperKey, claudeKey, cb, tracker);
  const totalArts = orgs.reduce((s, o) => s + (arts[o]||[]).length, 0);
  await sectionCb?.({ id: 'sov', title: 'AQ Press Analytics', summary: `${totalArts} total articles — ${orgs.slice(0,5).map(o=>`${o}: ${(arts[o]||[]).length}`).join(', ')}${orgs.length>5?' +more':''}` });
  ...
  const [xResults, igResults, liResults, ytResults] = await Promise.all([
    twitterCollector.run(orgs, dateFrom, dateTo, apidirKey, claudeKey, cb),
  ...
  await sectionCb?.({ id: 'social', title: 'Social & YouTube', summary: orgs.slice(0,4).map(o=>{const s=socialResults[o]||{};return `${o}: X ${s.x?.aqPosts||0} · IG ${s.ig?.aqPosts||0} · LI ${s.li?.totalPosts||0} · YT ${s.yt?.videoCount||0}`;}).join(' | ') });
  ...
  const topicAgg = {};
  orgs.forEach(o => Object.entries(data[o]?.topicCounts||{}).forEach(([t,n])=>{topicAgg[t]=(topicAgg[t]||0)+n;}));
  const topTopics = Object.entries(topicAgg).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([t,n])=>`${t} (${n})`).join(', ');
  await sectionCb?.({ id: 'topics', title: 'Topic Ownership', summary: topTopics || 'Classification complete — no dominant topics found' });
  ...
  await sectionCb?.({ id: 'momentum', title: 'Coverage Momentum', summary: `${spikeAnnotations.length} coverage spike${spikeAnnotations.length!==1?'s':''} detected across the tracked period` });

  // 5. AEO probing
  cb?.('[5/8] AEO — probing LLM visibility…');
  const aeoResults = await runAEO(orgs, openaiKey, perplexityKey, geminiKey, cb, tracker);
  const aeoLine = orgs.map(o=>`${o}: ${aeoResults[o]?.mentions||0}`).sort((a,b)=>(b.split(':')[1]||0)-(a.split(':')[1]||0)).slice(0,5).join(' · ');
  await sectionCb?.({ id: 'aeo', title: 'AEO / LLM Visibility', summary: `LLM mention counts — ${aeoLine||'No AEO data (optional keys not set)'}` });

  // 6. White-space gaps
  cb?.('[6/8] Finding white-space gaps (Claude Haiku)…');
  const emerging = await findGaps(orgs, dateFrom, dateTo, serperKey, claudeKey, arts, tracker, cb);
  cb?.(`  Found ${emerging.length} gap(s)`, 'ok');
  await sectionCb?.({ id: 'gaps', title: 'White-Space Gaps', summary: emerging.length > 0 ? `${emerging.length} emerging gap${emerging.length!==1?'s':''}: ${emerging.map(e=>e.topic).join(', ')}` : 'No significant coverage gaps found' });
  ...
  await sectionCb?.({ id: 'exec', title: 'Executive Summary', summary: execFindings[0]?.headline || 'Executive analysis complete' });
  const actions      = await genActionMatrix(orgs, data, claudeKey, tracker, cb);
  await sectionCb?.({ id: 'actions', title: 'Action Matrix', summary: `${actions.length} strategic recommendation${actions.length!==1?'s':''} across ${orgs.length} organisation${orgs.length!==1?'s':''}` });
